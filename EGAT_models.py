...
class SpGAT(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, alpha, nheads):
        ...
        self.output_module = nn.Sequential(
            ...
            nn.ReLU(),
            nn.LayerNorm(embed_size),
            nn.Linear(embed_size, nclass, bias=False)
        )
    ...
    def forward(self, x, edgeA, edgeB, edge_feat):
        ...
        x_residual_1 = x.clone()
        new_edge = torch.cat([att(x, edgeB, edge_feat)[1] for att in self.attentions_v_to_u], dim=1)
        x = torch.cat([att(x, edgeB, edge_feat)[0] for att in self.attentions_v_to_u], dim=1)
        x = self.out_att_v_to_u(x, edgeB, edge_feat)
        x = x + x_residual_1
        new_edge = torch.mean(new_edge, dim=1).reshape(new_edge.size()[0], 1)
        x_residual_2 = x.clone()
        new_edge_ = torch.cat([att(x, edgeA, new_edge)[1] for att in self.attentions_u_to_v], dim=1)
        x = torch.cat([att(x, edgeA, new_edge)[0] for att in self.attentions_u_to_v], dim=1)
        x = self.out_att_u_to_v(x, edgeA, new_edge)
        x = x + x_residual_2
