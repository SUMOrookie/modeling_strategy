...
class SpGraphAttentionLayer(nn.Module):
    ...
    def forward(self, node, edge, edge_feature):
        # dv = 'cuda:1' if node.is_cuda else 'cpu'
        dv = 'cpu'
        N = node.size()[0]
        edge = edge.t()
        ...
        assert not torch.isinf(edge_h).any()
        # edge: (2*D + 1) x E

        score = self.a.mm(edge_h).squeeze()
        score = self.leakyrelu(score)
        score = (score - score.mean()) / (score.std() + 1e-6)  # 标准化
        # print(f"Attention score min={score.min().item()}, max={score.max().item()}")
        assert not torch.isinf(score).any(), "score 中有 Inf"
        assert not torch.isnan(score).any(), "score 中有 NaN"

        # edge_e = torch.exp(-self.leakyrelu(self.a.mm(edge_h).squeeze())) # 每一条边的“注意力权重”
        edge_e = torch.exp(-score) # 每一条边的“注意力权重”
        ...
        # 确保 b 在正确的设备上
        b = torch.ones(size=(N, 1), device=model_device)
