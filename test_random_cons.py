...
if __name__ == '__main__':
    ...
    for repair_method in repair_method_list:
        ...
        for seed in seed_list:
            random.seed(seed)
            df,every_second_gap_agg = accelerated_solving(cache, lp_files_dir, solve_num, agg_num, seed, problem, repair_method,
                                                          agg_model_solve_time, time_limit, neighborhood, Threads, post_solve_method
                                                          ,KEEP_FRACTION,NORM_METHOD,SIMILARITY_METRIC)
            df.to_csv(result_dir + f"/random_aggnum_{agg_num}_seed_{seed}_repair_{repair_method}_{post_solve_method}.csv", index=False)
            all_runs.append(df)
