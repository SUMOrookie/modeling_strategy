...
def repair_solution(lp_path,Threads,repair_method,vaule_dict):
    ...
    elif repair_method == "subproblem_cons_geq":
        vaule_dict = heuristic_repair_subproblem_cons_geq(repair_model, vaule_dict)
    ...
    return repair_model,vaule_dict
...
def heuristic_repair_subproblem_cons_geq(repair_model,value_dict):
    print("------------repair-------------")

    # 复制模型，用来构造子问题
    sub = repair_model.copy()

    # 从value_dict中筛选出取值为1的变量，添加约束固定为1（1<=x<=1）
    one_vars = [v for v in sub.getVars() if value_dict.get(v.VarName, 0) == 1]
    for v in one_vars:
        # 固定变量为1（等价于添加约束 v >=1 且 v <=1）
        sub.addConstr(v == 1, name=f"fix_{v.VarName}_to_1")
    sub.update()  # 更新模型以生效约束
...
def heuristic_repair_subproblem(repair_model,value_dict):
    ...
    ones = [v for v in sub.getVars() if value_dict.get(v.VarName, 0) == 1]
    for v in zeros:
        sub.remove(v)
    sub.update()
...
def heuristic_repair_subproblem_scip(sub, value_dict):
    print("------------repair-------------")

    # sub = fix_model

    # 2. 删除 value_dict 中值为 0 的变量
    zeros = [v for v in sub.getVars() if value_dict.get(v.name, 0) == 0]
    for v in zeros:
        for cons in list(sub.getConss()):
            vars = sub.getValsLinear(cons).keys()# 用 list 防止迭代时修改
            if v.name in vars:
                sub.chgCoefLinear(cons, v, 0)   # 将系数改为0，相当于移除变量
                break
        sub.delVar(v)

    # 3. 删除空约束
    for cons in sub.getConss():
        # getVarsLinear 返回约束线性部分涉及的变量列表（最稳妥）
        vars_in_row = sub.getVars(cons)
        if len(vars_in_row) == 0:
            sub.delCons(cons)
    ...
    if status in ["optimal", "feasible"]:
        for v in sub.getVars():
            sol_val = sub.getVal(v)
            value_dict[v.name] = int(round(sol_val))
...
def PostSolve(repair_model,neighborhood,vaule_dict,lp_file,start_time,time_limit,method,reduced_costs=None,callback=None):
    ...
    if method == "neighborhood_improved":
        # 1. 校验输入长度一致性（避免索引错位）
        if len(repair_Vars) != len(reduced_costs) or len(repair_Vars) != len(vaule_dict):
            raise ValueError("变量列表、reduced_costs、vaule_dict的长度必须一致！")

        # 2. 构建(变量对象, 变量值, reduced cost)的列表（核心：保留变量对象）
        var_val_rc_list = []
        for idx,var in enumerate(repair_Vars):
            var_name = var.VarName  # 仅用于匹配字典值，不替换变量对象
            var_value = vaule_dict.get(var_name, None)
            rc = reduced_costs[idx]

            if var_value not in (0, 1):
                raise ValueError(f"变量{var_name}的值{var_value}不是0/1，请检查vaule_dict！")
            # 列表中始终存放：(变量对象, 变量值, reduced cost)
            var_val_rc_list.append((var, var_value, rc))

        # 3. 拆分两组变量：值为0的组 + 值为1的组（元素仍是(变量对象, 值, rc)）
        var_0_group = [item for item in var_val_rc_list if item[1] == 0]
        var_1_group = [item for item in var_val_rc_list if item[1] == 1]

        # 4. 差异化排序：仅按rc排序
        var_0_group_sorted = sorted(var_0_group, key=lambda x: x[2], reverse=False) 
        var_1_group_sorted = sorted(var_1_group, key=lambda x: x[2], reverse=True)
        # var_0_group_sorted = sorted(var_0_group, key=lambda x: x[2], reverse=True) 
        # var_1_group_sorted = sorted(var_1_group, key=lambda x: x[2], reverse=False)

        # 5. 合并排序后的列表（元素仍是(变量对象, 值, rc)）
        var_val_rc_sorted = var_0_group_sorted + var_1_group_sorted

        # 6. 提取排序后的变量对象列表（核心修正：取元组第一个元素——变量对象）
        repair_Vars = [item[0] for item in var_val_rc_sorted]
    for idx in range(len(repair_Vars)):
        ...
        if vaule_dict[varname] != None:
            if method == "warm_start":
            ...
            elif method == "neighborhood" or method == "neighborhood_improved":
                # 作为启动值
                # repair_model.getVarByName(varname).Start = vaule_dict[varname]
                # 加邻域的固定
                ...
                if vaule_dict[varname] == 1.0 and k1_cnt < k1:
                    var = repair_model.getVarByName(varname)
                    ...
                    k1_cnt += 1
                    delta_var_list.append(var_delta)
                if k0_cnt >= k0 and k1_cnt >= k1:
                    # k0与k1已经达到上限
                    break
            else:
                raise Exception("unknown post solve method")
    ...
    # 求解,计算指标
    metrics_list = []
    cb = utils.make_callback(lp_file, metrics_list)
    ...
    if callback is not None:
        repair_model.optimize(callback) # 这个是app.py里传入的callback，主要是为了在求解过程中记录日志指标，绘图用的。
    else:
        repair_model.optimize(cb) # 这个是test.py里传入的callback，主要是为了在求解过程中记录相关的指标。
    return metrics_list
...
def repair_scip(fix_model,vaule_dict,repair_method,lp_path):
    if repair_method == "naive":
        # 简单的启发式修复
        raise Exception("还没实现")
        vaule_dict = heuristic_repair(fix_model, vaule_dict)
    elif repair_method == "score":
        # 变量评分
        raise Exception("还没实现")
        vaule_dict = heuristic_repair_with_score(fix_model, vaule_dict)
    elif repair_method == "subproblem":
        vaule_dict = heuristic_repair_subproblem_scip(fix_model, vaule_dict)
    elif repair_method == "lightmilp":
        raise Exception("还没实现")
        vaule_dict = heuristic_repair_light_MILP(fix_model, vaule_dict, lp_path)
    else:
        raise Exception("unknown repair_method")
    return vaule_dict