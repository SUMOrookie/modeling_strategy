...
import numpy as np
import time
# from trainPredictModel import DEVICE
...
class GraphDataset(torch_geometric.data.Dataset):
    ...
    def get(self, index):
        ...
        constraint_features = torch.where(torch.isnan(constraint_features), torch.tensor(1.0, device=constraint_features.device), constraint_features)
        ...
        graph = BipartiteNodeData(
            torch.tensor(constraint_features, dtype=torch.float32, device=DEVICE),
            torch.tensor(edge_indices, dtype=torch.long, device=DEVICE),  # 替换LongTensor
            torch.tensor(edge_features, dtype=torch.float32, device=DEVICE),  # 替换FloatTensor
            torch.tensor(variable_features, dtype=torch.float32, device=DEVICE),  # 替换FloatTensor
        )
