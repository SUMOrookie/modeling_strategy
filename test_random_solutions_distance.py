...
# k1 = 100
# Delta = 10
def cal_distance(cache:dict, directory: str, num_problems: int, agg_num: int, seed:int, problem:str, repair_method:str,
                 agg_model_solve_time, time_limit,neighborhood
                 ,Threads,post_solve_method,KEEP_FRACTION,NORM_METHOD,SIMILARITY_METRIC):
    # 获取目录下所有的 .lp 文件
    ...
    for lp_file in lp_files:
        ...
        print(f"distance:{distance}\t percent:{distance/len(vaule_dict)}")
        #### 邻域搜索（保证最优性）
...
def accelerated_solving(cache:dict, directory: str, num_problems: int, agg_num: int, seed:int, problem:str, repair_method:str, agg_model_solve_time, time_limit,neighborhood,Threads,post_solve_method):
    # 获取目录下所有的 .lp 文件
    lp_files = [f for f in os.listdir(directory) if f.endswith('.lp')]
    lp_files.sort()  # 按文件名排序，确保顺序一致
...
if __name__ == '__main__':
    ...
    problem = "CA"
    json_file_path = 'parameters/param_large.json'
    ...
    agg_model_solve_time = 4
    time_limit = 1000
    neighborhood = {"k0":300,"k1":300,"Delta":200}
    ...
    for repair_method in repair_method_list:
        ...
        result_dir = f"./result/{KEEP_FRACTION}_{NORM_METHOD}_{SIMILARITY_METRIC}_random_{data_dir}_solve_{solve_num}_aggNum_{agg_num}_aggsolvetime_{agg_model_solve_time}_repair_{repair_method}_{post_solve_method}_{neighborhood_str}_distance"
        ...
        for seed in seed_list:
            random.seed(seed)
            post_solve_method = "neighborhood"
            df,every_second_gap_agg = cal_distance(cache, lp_files_dir, solve_num, agg_num, seed, problem,
                                                   repair_method, agg_model_solve_time, time_limit, neighborhood,
                                                   Threads, post_solve_method,KEEP_FRACTION,NORM_METHOD,SIMILARITY_METRIC)
            # df,every_second_gap_agg = accelerated_solving(cache, lp_files_dir, solve_num, agg_num, seed, problem, repair_method, agg_model_solve_time, time_limit, neighborhood, Threads, post_solve_method)
            df.to_csv(result_dir + f"/random_aggnum_{agg_num}_seed_{seed}_repair_{repair_method}_{post_solve_method}_distance.csv", index=False)
            all_runs.append(df)
